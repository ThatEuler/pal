...
local function combat()
	...
	if target.alive and target.enemy and not player.channeling() then
		...
		-- Summon Felguard
		if not pet.exists and castable(SB.SummonFelguard) and -spell(SB.SummonFelguard) == 0 and not UnitInVehicle('player') and not player.spell(SB.SummonFelguard).lastcast then
