...
local function combat()
  if target.alive and target.enemy and player.alive and not player.channeling() then
    ...
    if talent(7, 1) then
      ...
      if
        castable(SB.FelBarrage, "target") and -spell(SB.FelBarrage) == 0 and player.buff(SB.Metamorphosis).up and
          (enemyCount >= 2 or toggle("multitarget")) and
          talent(3, 3)
       then
      ...
      if
        castable(SB.DeathSweep, "target") and -spell(SB.DeathSweep) == 0 and
          (talent(5, 2) or (talent(3, 1) and (enemyCount >= 2 or toggle("multitarget"))))
       then
      ...
      if
        castable(SB.BladeDance, "target") and -spell(SB.BladeDance) == 0 and
          (talent(5, 2) or (talent(3, 1) and (enemyCount >= 2 or toggle("multitarget")))) and
          -spell(SB.Metamorphosis) > 0 and
          target.distance < 8
       then
      ...
      if
        castable(SB.Felblade, "target") and -spell(SB.Felblade) == 0 and
          (power.fury.actual < 40 or (not player.buff(SB.Metamorphosis).up and power.fury.actual <= 60)) and
          talent(1, 3)
       then
      ...
      if
        modifier.shift and castable(SB.EyeBeam, "target") and -spell(SB.EyeBeam) == 0 and
          (not talent(1, 1) or (not player.buff(SB.Metamorphosis).up and power.fury.actual <= 50))
       then
      ...
      if
        castable(SB.Annihilation, "target") and -spell(SB.Annihilation) == 0 and
          (talent(1, 1) or power.fury.actual > 70 or player.buff(SB.Metamorphosis).remains < 5) and
          (talent(5, 2) and power.fury.actual >= 55)
       then
      ...
      if
        castable(SB.ChaosStrike, "target") and -spell(SB.ChaosStrike) == 0 and
          (talent(1, 1) or power.fury.actual > 70 and -spell(SB.Metamorphosis) > 6 and power.fury.actual > 70) and
          (talent(5, 2) and power.fury.actual >= 55)
       then
      ...
      if
        castable(SB.FelRush, "target") and -spell(SB.FelRush) == 0 and talent(2, 2) and -spell(SB.EyeBeam) > 0 and
          spell(SB.FelRush).charges == 2
       then
      ...
      if
        castable(SB.ThrowGlaive, "target") and -spell(SB.ThrowGlaive) == 0 and
          ((target.distance >= 8 and target.distance <= 30) or talent(2, 2))
       then
    ...
    if target.debuff(SB.Nemesis).up then
      ...
      if
        modifier.shift and castable(SB.EyeBeam, "target") and -spell(SB.EyeBeam) == 0 and not talent(1, 1) and
          not talent(5, 3) and
          (talent(5, 2) and power.fury.actual >= 45)
       then
        return cast(SB.EyeBeam)
      end
      ...
      if
        castable(SB.Annihilation, "target") and -spell(SB.Annihilation) == 0 and
          (talent(2, 2) or power.fury.actual > 70 or player.buff(SB.Metamorphosis).remains < 5) and
          (talent(5, 2) and power.fury.actual >= 55)
       then
        return cast(SB.Annihilation)
      end
      ...
      if
        castable(SB.ChaosStrike, "target") and -spell(SB.ChaosStrike) == 0 and
          (talent(2, 2) or power.fury.actual > 70 or -spell(SB.Metamorphosis) > 6) and
          (talent(5, 2) and power.fury.actual >= 55)
       then
        return cast(SB.ChaosStrike)
      end
    ...
    if talent(7, 2) then
      ...
      if
        castable(SB.FelBarrage, "target") and -spell(SB.FelBarrage) == 0 and talent(3, 3) and
          player.buff(SB.Momentum).up
       then
        return cast(SB.FelBarrage, "target")
      end
  ...
  local settings = {
    ...
    template = {
      ...
      {type = "text", text = "Defensive Settings"},
      {
        key = "NWHealth",
        type = "spinner",
        text = "Netherwalker at Health %",
        default = "60",
        desc = "cast Netherwalker at",
        min = 0,
        max = 100,
        step = 1
      },
      ...
      },
      {
        key = "DHealth",
        type = "spinner",
        text = "Darkness at Health %",
        default = "60",
        desc = "cast Darkness at",
        min = 0,
        max = 100,
        step = 1
      },
