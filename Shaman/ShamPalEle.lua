...
local function combat()
  ...
  if target.alive and target.enemy then
    ...
    if
      talent(7, 2) and toggle("cooldowns") and -spell(SB.StormKeeper) == 0 and
        (inRange >= 3 or (UnitClassification("target") == "worldboss" or UnitClassification("target") == "rareelite"))
     then
    ...
    if
      toggle("multitarget", false) and inRange <= 2 and target.castable(SB.LavaBurst) and
        target.debuff(SB.FlameShock).up
     then
      return cast(SB.LavaBurst, target)
    end

    if toggle("multitarget", false) and inRange >= 2 then
      if castable(SB.ChainLightning) then
        return cast(SB.ChainLightning, target)
      end
    elseif toggle("multitarget", false) and inRange == 1 or toggle("multitarget", true) then
      if castable(SB.LightningBolt) then
