...
local function combat()
  if target.alive and target.enemy and player.alive and not player.channeling() then
    ...
    -- Storm Bolt
    if
      castable(SB.StormBolt, "target") and -spell(SB.StormBolt) == 0 and target.interrupt(intpercent, false) and
        StormBoltInt and
    ...
    if enemyCount == 1 then
      ...
      if -target.health >= 20 or (talent(3, 1) and -target.health >= 35) then
        ...
        if
          castable(SB.Skullsplitter, "target") and -spell(SB.Skullsplitter) == 0 and -spell(SB.Bladestorm) > 12 and
            -power.rage < 60 and
            talent(1, 3)
         then
      ...
      if -target.health < 20 or (talent(3, 1) and -target.health < 35) then
        ...
        if
          castable(SB.MortalStrike, "target") and -spell(SB.MortalStrike) == 0 and buff(SB.Overpower).count == 2 and
            talent(7, 2) and
            -power.rage >= 30
         then
          return cast(SB.MortalStrike)
    ...
    if enemyCount >= 4 then
      ...
      if
        castable(SB.MortalStrike, "target") and -spell(SB.MortalStrike) == 0 and -power.rage >= 30 and
      ...
      if
        castable(SB.Whirlwind, "target") and -spell(SB.Whirlwind) == 0 and -power.rage >= 30 and
