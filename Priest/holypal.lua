...
local function combat()
  ...
  if not player.channeling() then
    ...
    if player.buff(SB.Apotheosis).up then
      if
        lowest.castable(SB.FlashHeal) and lowest.health.effective <= apotheosisflash and
          spell(SB.HolyWordSerenity).cooldown >= apotheosisserenetiycd
       then
        return cast(SB.FlashHeal, lowest)
      elseif
        tank.castable(SB.FlashHeal) and tank.health.effective <= apotheosisflash and
          spell(SB.HolyWordSerenity).cooldown >= apotheosisserenetiycd
       then
    ...
    elseif
      guardianspirittarget == "gs_all" and lowest.castable(SB.GuardianSpirit) and
        lowest.health.percent <= guardianspirit
     then
    ...
    if movingrenews == "renew_always" then
      if
        lowest.castable(SB.Renew) and lowest.health.effective <= renewlowest and max_renews <= simultaneousrenews and
          lowest.buff(SB.Renew).down
       then
        return cast(SB.Renew, lowest)
      elseif
        tank.castable(SB.Renew) and tank.health.effective <= renewtank and max_renews <= simultaneousrenews and
          tank.buff(SB.Renew).down
       then
    ...
    if movingrenews == "renew_moving" then
      if
        lowest.castable(SB.Renew) and lowest.health.effective <= renewlowest and max_renews <= simultaneousrenews and
          lowest.buff(SB.Renew).down and
          player.moving
       then
        return cast(SB.Renew, lowest)
      elseif
        tank.castable(SB.Renew) and tank.health.effective <= renewtank and max_renews <= simultaneousrenews and
          tank.buff(SB.Renew).down and
          player.moving
       then
        return cast(SB.Renew, tank)
      end
    ...
    if
      lowest.castable(SB.FlashHeal) and player.buff(SB.SurgeofLight).up and player.buff(SB.SurgeofLight).remains > 3 and
        lowest.health.effective <= flashhealsurge
     then
      return cast(SB.FlashHeal, lowest)
    elseif
      lowest.castable(SB.FlashHeal) and player.buff(SB.SurgeofLight).up and player.buff(SB.SurgeofLight).remains < 3 and
        lowest.health.effective <= flashhealsurgeemergency
     then
      return cast(SB, FlashHeal, lowest)
    end

    if
      tank.castable(SB.FlashHeal) and player.buff(SB.SurgeofLight).up and player.buff(SB.SurgeofLight).remains > 3 and
        tank.health.effective <= flashhealsurge
     then
      return cast(SB.FlashHeal, tank)
    elseif
      tank.castable(SB.FlashHeal) and player.buff(SB.SurgeofLight).up and player.buff(SB.SurgeofLight).remains < 3 and
        tank.health.effective <= flashhealsurgeemergency
     then
      return cast(SB, FlashHeal, tank)
    end
  ...
  local settings = {
    ...
    template = {
      ...
      {key = "fade", type = "spinner", text = "Fade", desc = "Health % to cast at", min = 1, max = 100, step = 5},
      {
        ...
        type = "spinner",
        text = "Heal",
        desc = "Health % of lowest in Group to Cast at",
        default = 70,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "prayerofhealingpercent",
        type = "spinner",
        text = "Prayer of Healing",
        desc = "Health % of Group to Cast at",
        default = 70,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "prayerofhealingnumberofplayer",
        type = "spinner",
        text = "Prayer of Healing Targets",
        desc = "Number of damaged players near you",
        default = 3,
        min = 1,
        max = 6,
        step = 1
      ...
      {
        key = "simultaneousrenews",
        type = "spinner",
        text = "Max Renews",
        desc = "Number of Max Simulataneous Renews",
        default = 6,
        min = 1,
        max = 40,
        step = 1
      ...
      },
      {
        key = "flashhealsurge",
        type = "spinner",
        text = "Flash Heal Surge of Light",
        desc = "Health % of lowest in Group to cast at under Surge of Light",
        default = 75,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "flashhealsurgeemergency",
        type = "spinner",
        text = "Flash Heal Surge of Light Emergency",
        desc = "Health % to not Waste SurgeofLight",
        default = 80,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "desperateprayerpercent",
        type = "spinner",
        text = "Desperate Prayer",
        desc = "Health % of Player to Cast at",
        default = 35,
        min = 5,
        max = 100,
        step = 5
      ...
      {type = "text", text = "Apotheosis Settings"},
      {
        key = "apotheosisflash",
        type = "spinner",
        text = "Apotheosis Flash Heal",
        desc = "Use Flash Heal while Apotheosis is active",
        default = 85,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "apotheosisserenetiycd",
        type = "spinner",
        text = "Apotheosis Serenity CD",
        desc = "Use Flash Heal only if Serenity CD is above this",
        default = 15,
        min = 5,
        max = 60,
        step = 5
      },
      {
        key = "apotheosispoh",
        type = "spinner",
        text = "Apotheosis Prayer of Healing",
        desc = "Use PoH while Apotheosis is active",
        default = 90,
        min = 5,
        max = 100,
        step = 5
      ...
      {
        key = "apotheosispohplayers",
        type = "spinner",
        text = "Apotheosis PoH Targets",
        desc = "Number of damaged players near you",
        default = 3,
        min = 1,
        max = 6,
