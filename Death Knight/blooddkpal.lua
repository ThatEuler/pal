...
local function combat()
  if target.alive and target.enemy and player.alive and not player.channeling() then
    ...
    --Mind Freeze
    if
      toggle("interrupts", false) and castable(SB.MindFreeze, "target") and -spell(SB.MindFreeze) == 0 and
    ...
    --Asphyxiate
    if
      toggle("interrupts", false) and castable(SB.Asphyxiate, "target") and -spell(SB.Asphyxiate) == 0 and
        -spell(SB.MindFreeze) > 1 and
        target.interrupt(intpercent, false) and
        target.distance < 20
     then
    ...
    if
      castable(SB.Marrowrend) and -spell(SB.Marrowrend) == 0 and player.buff(SB.BoneShield).remains <= 3 and
        player.power.runes.count >= 2
     then
    ...
    if
      castable(SB.DeathStrike) and -spell(SB.DeathStrike) == 0 and player.power.runicpower.actual >= 45 and
        -player.health <= DSHealth
     then
    ...
    if
      castable(SB.Blooddrinker) and -spell(SB.Blooddrinker) == 0 and player.buff(SB.DancingRuneWeapon).down and
        talent(1, 2)
     then
    ...
    if
      castable(SB.BloodBoil) and -spell(SB.BloodBoil) == 0 and
        (spell(SB.BloodBoil).charges == 2 or not (-target.debuff(SB.BloodPlague)))
     then
    ...
    if
      castable(SB.Marrowrend) and -spell(SB.Marrowrend) == 0 and player.buff(SB.BoneShield).count <= 7 and
        player.power.runes.count >= 2 and
        player.buff(SB.DancingRuneWeapon).down
     then
      return cast(SB.Marrowrend, "target")
    end
    ...
    if
      castable(SB.Marrowrend) and -spell(SB.Marrowrend) == 0 and player.buff(SB.BoneShield).count <= 4 and
        player.power.runes.count >= 2 and
        player.buff(SB.DancingRuneWeapon).up
     then
      return cast(SB.Marrowrend, "target")
    end
    ...
    if
      castable(SB.RuneStrike) and -spell(SB.RuneStrike) == 0 and talent(1, 3) and spell(SB.RuneStrike).charges == 2 and
        player.power.runes.count <= 3
     then
    ...
    if
      castable(SB.DeathandDecay) and -spell(SB.DeathandDecay) == 0 and player.buff(SB.CrimsonScourge).up and
        target.distance <= 8
     then
      return cast(SB.DeathandDecay, "player")
    end
  ...
  local settings = {
    ...
    template = {
      ...
      {type = "text", text = "Defensive Settings"},
      {
        key = "DSHealth",
        type = "spinner",
        text = "Death Strike at Health %",
        default = "80",
        desc = "cast Death Strike at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "MOBHealth",
        type = "spinner",
        text = "Mark of Blood at Health %",
        default = "75",
        desc = "cast Mark of Blood at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "RTHealth",
        type = "spinner",
        text = "Rune Tap at Health %",
        default = "70",
        desc = "cast Rune Tap at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "AMSHealth",
        type = "spinner",
        text = "Anti Magic Shell at Health %",
        default = "60",
        desc = "cast Anti Magic Shell at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "VBHealth",
        type = "spinner",
        text = "Vampiric Blood at Health %",
        default = "40",
        desc = "cast Vampiric Blood at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "IFHealth",
        type = "spinner",
        text = "Icebound Fortitude at Health %",
        default = "20",
        desc = "cast Icebound Fortitude at",
        min = 0,
        max = 100,
        step = 1
      }
