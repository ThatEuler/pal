...
local function combat()
  ...
  -- Moonkin Form
  if
    not modifier.lalt and not lastcast(SB.MoonkinForm) and player.buff(SB.TigerDashBuff).down and
      GetShapeshiftForm() ~= 4
   then
  ...
  -- rotation
  if
    not target.castable(SB.Starsurge) and target.castable(SB.SolarWrath) and player.buff(SB.LunarEmpowerment).count == 0 and
  ...
  -----------------------------
  if
    not talent(5, 3) and -spell(SB.Berserking) == 0 and
      (player.buff(SB.CelestialAlignment).up or -spell(SB.CelestialAlignment) > 30)
   then
    cast(SB.Berserking)
  end
  if
    talent(5, 3) and -spell(SB.Berserking) == 0 and
      (player.buff(SB.IncarnationBalance).up or -spell(SB.IncarnationBalance) > 30)
   then
    cast(SB.Berserking)
  end
  ...
  if toggle("cooldowns", false) then
    if
      talent(5, 3) and power.astral.actual > 40 and target.debuff(SB.StellarFlare).up and
        -spell(SB.IncarnationBalance) == 0
     then
      return cast(SB.IncarnationBalance)
    elseif
      talent(5, 2) and player.buff(SB.Starlord).count >= 2 and power.astral.actual > 40 and
        -spell(SB.CelestialAlignment) == 0
     then
    ...
    if
      talent(7, 2) and talent(5, 3) and -player.spell(SB.FuryofElune) == 0 and
        (player.buff(SB.IncarnationBalance).up or -spell(SB.IncarnationBalance) > 30)
     then
      return cast(SB.FuryofElune, "target")
    end
    if
      talent(7, 2) and not talent(5, 3) and -player.spell(SB.FuryofElune) == 0 and
        (player.buff(SB.CelestialAlignment).up or -spell(SB.CelestialAlignment) > 30)
     then
      return cast(SB.FuryofElune, "target")
    end
  ...
  if
    talent(6, 3) and target.castable(SB.StellarFlare) and not lastcast(SB.StellarFlare) and
      ((target.time_to_die / (1.5 / ((UnitSpellHaste("player") / 100) + 1))) * enemyCount) >= 5 and
  ...
  if
    target.castable(SB.Sunfire) and
      ((target.time_to_die * (enemyCount / (1.5 / ((UnitSpellHaste("player") / 100) + 1)))) >= (4 + enemyCount)) and
      (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 3.6)
   then
    return cast(SB.Sunfire, "target")
  end
  if
    target.castable(SB.Moonfire) and
      ((target.time_to_die / (1.5 / ((UnitSpellHaste("player") / 100) + 1))) * enemyCount) >= 6 and
      (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 4.8)
   then
    return cast(SB.Moonfire, "target")
  end
  ...
  elseif
    target.castable(SB.LunarStrike) and power.astral.deficit >= 16 and player.buff(SB.LunarEmpowerment).count == 2 and
      enemyCount < 3 and
      power.astral.active >= 40 and
      player.buff(SB.SolarEmpowerment) == 3
   then
    return cast(SB.LunarStrike, "target")
