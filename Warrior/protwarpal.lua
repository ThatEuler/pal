...
local function combat()
  ...
  if
    toggle("interrupts", false) and target.interrupt(intpercent) and target.distance < 8 and -spell(SB.Rebuke) > 0 and
      -spell(SB.BlindingLight) == 0
   then
    return cast(SB.BlindingLight, "target")
  end
  if
    autoStun and toggle("interrupts", false) and target.interrupt(intpercent) and target.distance < 8 and
      -spell(SB.Rebuke) > 0 and
      -spell(SB.BlindingLight) > 0 and
  ...
  if
    talent(SB.BastionofLight) and spell(SB.ShieldoftheRighteous).charges == 0 and toggle("cooldowns", false) and
      -spell(SB.BastionofLight) == 0
   then
  ...
  if useTrinkets then
    ...
    --Jes Howler (159622)
    if
      Trinket13 == 159622 and target.enemy and target.distance < 8 and target.health.percent > 50 and
        GetItemCooldown(159627) == 0 and
        -spell(SB.AvengingWrath) > 10 and
        player.buff(SB.AvengingWrath).down
     then
      macro("/use 13")
    end
    if
      Trinket14 == 159622 and target.enemy and target.distance < 8 and target.health.percent > 50 and
        GetItemCooldown(159627) == 0 and
        -spell(SB.AvengingWrath) > 10 and
        player.buff(SB.AvengingWrath).down
     then
    ...
    if
      Trinket13 == 159611 and target.enemy and target.distance < 8 and target.health.percent > 50 and
        GetItemCooldown(159611) == 0 and
        -spell(SB.AvengingWrath) > 10 and
        player.buff(SB.AvengingWrath).down
     then
      macro("/use [@player] 13 ")
    ...
    if
      Trinket14 == 159611 and target.enemy and target.distance < 8 and target.health.percent > 50 and
        GetItemCooldown(159611) == 0 and
        -spell(SB.AvengingWrath) > 10 and
        player.buff(SB.AvengingWrath).down
     then
  ...
  if
    spell(SB.ShieldoftheRighteous).charges > 0 and target.distance < 8 and not (-buff(SB.ShieldoftheRighteousBuff)) and
      not (-buff(SB.InnerLight))
   then
  ...
  if
    spell(SB.ShieldoftheRighteous).charges > 0 and target.distance < 8 and not (-buff(SB.ShieldoftheRighteousBuff)) and
      not (-buff(SB.InnerLight)) and
      -spell(SB.Seraphim) > 16
   then
  ...
  if
    toggle("BoP", false) and lowest.castable(SB.BlessingofProtection) and lowest.health.percent <= 20 and
      lowest.debuff(SB.Forbearance).down and
      lowest ~= tank and
      lowest ~= player
   then
  ...
  if
    BoS_Others and not talent(4, 3) and lowest.castable(SB.BlessingofSacrifice) and lowest.health.percent <= 40 and
      lowest ~= tank and
      lowest ~= player
   then
  ...
  if
    LoH_Others and lowest.castable(SB.LayonHands) and lowest.debuff(SB.Forbearance).down and lowest.health.percent <= 15
   then
  ...
  if
    -player.health < GuardianofAncientKings_health and castable(SB.GuardianofAncientKings) and
      -spell(SB.ArdentDefender) > 0 and
      -spell(SB.GuardianofAncientKings) == 0
   then
  ...
  if toggle("autoTaunt", false) and -spell(SB.HandofReckoning) == 0 then
    ...
    if
      target.enemy and IsSpellInRange("HandofReckoning", "target") and target.combat and
        (isTanking == 0 or isTanking == nil)
     then
...
local function interface()
  local settings = {
    ...
    template = {
      ...
      {
        key = "ArdentDefender_health",
        type = "checkspin",
        text = "Ardent Defender",
        desc = "Auto use Ardent Defender at health %",
        min = 5,
        max = 100,
        step = 5
      },
      {
        key = "GuardianofAncientKings_health",
        type = "checkspin",
        text = "Guardian of Ancient Kings",
        desc = "Auto use Guardian of Ancient Kings at health %",
        min = 5,
        max = 100,
        step = 5
      },
