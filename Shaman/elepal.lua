...
local function combat()
  ...
  if modifier.shift then
    if
      talent(4, 3) and toggle("cooldowns") and -spell(SB.LiquidMagmaTotem) == 0 and
        not player.spell(SB.LiquidMagmaTotem).lastcast
     then
  ...
  if target.alive and target.enemy then
    ...
    -- Interupts
    if
      toggle("interrupts", false) and target.castable(SB.WindShear) and target.interrupt(math.random(25, 75)) and
        target.distance <= 30
     then
    ...
    if player.moving then
      ...
      elseif
        target.castable(SB.Flameshock) and
    ...
    if
      target.castable(SB.Lightningbolt) and player.buff(SB.StormKeeper).up and
        player.buff(SB.StormKeeper).remains < current_gcd * 4 * player.buff(SB.StormKeeper).count
     then
    ...
    if enemyCount <= 2 then
      ...
      if target.castable(SB.Flameshock) then
        if
          target.debuff(SB.Flameshock).down or
            (talent(4, 2) and -spell(SB.Stormelemental) < 2 * current_gcd or
              target.debuff(SB.Flameshock).remains <= current_gcd or
              talent(7, 3) and target.debuff(SB.Flameshock).remains < (-spell(SB.Ascendance) + 15) and
                -spell(SB.Ascendance) < 4 and
                (not talent(4, 2) or talent(4, 2) and -spell(SB.Stormelemental) < 120)) and
              player.buff(SB.Windgust).count < 14 and
              player.buff(SB.Surgeofpower).down
         then
          return cast(SB.Flameshock, target)
        end
      end
      ...
      if talent(7, 3) and toggle("cooldowns", false) and -spell(SB.Ascendance) == 0 then
        if
          player.buff(SB.Bloodlust).up or
            -spell(SB.Lavaburst) > 0 and (not talent(4, 2) or talent(4, 2) and -spell(SB.Stormelemental) > 120) and
              (not talent(6, 3) or player.buff(SB.Icefury).down and -spell(SB.Icefury > 0))
         then
      ...
      --elemental blast
      if
        talent(1, 3) and target.castable(SB.ElementalBlast) and
          (talent(4, 1) and player.buff(SB.MasteroftheElements).up and -power.maelstrom < 60 or not talent(4, 1)) and
          (not (-spell(SB.Stormelemental) > 120 and talent(4, 2)) or
            az_nh and player.buff(SB.AzeriteNaturalHarmony).count == 3 and
              (player.buff(SB.windgust).down or player.buff(SB.Windgust).count < 14))
       then
      ...
      if target.castable(SB.Lightningbolt) then
        if
          player.buff(SB.StormKeeper).up and enemyCount < 2 and
            (player.buff(SB.MasteroftheElements).up and (not talent(6, 1) or player.buff(SB.Surgeofpower).up))
         then
          return cast(SB.Lightningbolt, target)
        end
      end
...
&expected_combat_length-time-cooldown.storm_elemental.remains-150*floor((expected_combat_length-time-cooldown.storm_elemental.remains)%150)>=30*(1+(azerite.echo_of_the_elementals.rank>=2)))
      ...
      if target.castable(SB.EarthShock) then
        if
          (talent(4, 1) and player.buff(SB.Surgeofpower).down and player.buff(SB.MasteroftheElements).up) or
            (-power.maelstrom >= (maelstrom_pool - 10)) or
            (player.buff(SB.StormKeeper).up and enemyCount < 2)
         then
          return cast(SB.Earthshock, target)
        ...
        elseif not talent(4, 1) and talent(2, 2) and (player.buff(SB.StormKeeper).up or -power.maelstrom >= 50) then
          return cast(SB.Earthshock, target)
        elseif
          talent(7, 2) and not (talent(4, 2) or -spell(SB.Stormelemental) > 120) and
            ((target.time_to_die) - (-spell(SB.Stormelemental)) -
              (150 * math.floor((target.time_to_die - (-spell(SB.Stormelemental)))) % 150) >=
              30 * (1 + echoCount) >=
              2)
         then
          return cast(SB.Earthshock, target)
      ...
      if target.castable(SB.FrostShock) then
        if
          talent(6, 3) and player.buff(SB.Icefury).up and -spell(SB.Lavaburst) > 0 and
            (player.buff(SB.Icefury).remains < current_gcd * 4 * player.buff(SB.Icefury).count) or
            player.moving
         then
      ...
      --re-dot
      if
        target.castable(SB.Flameshock) and enemyCount > 1 and
          (not target.debuff(SB.FlameShock) or target.debuff(SB.FlameShock).remains <= 6) and
          player.buff(SB.Surgeofpower).up
       then
        return cast(SB.Flameshock, target)
      elseif
        target.castable(SB.Flameshock) and enemyCount == 1 and
    ...
    if enemyCount >= 3 then
      ...
      if
        target.castable(SB.Flameshock) and
          (not target.debuff(SB.FlameShock) or target.debuff(SB.FlameShock).remains <= 6) and
          enemyCount < 5 and
          (not talent(4, 2) or (talent(4, 2) and -spell(SB.Stormelemental) < 120) or
            enemyCount == 3 and (player.buff(SB.Windgust).down or player.buff(SB.Windgust).count < 14))
       then
        return cast(SB.Flameshock, target)
      end
      --lave burst
      if
        target.castable(SB.Lavaburst) and (player.buff(SB.LavaSurge).up or player.buff(SB.Ascendance).up) and
          enemyCount < 4 and
          (not talent(4, 2) or (talent(4, 2) and -spell(SB.Stormelemental) < 120))
       then
        return cast(SB.Lavaburst, target)
      end
      --elemental blast
      if
        talent(1, 3) and target.castable(SB.ElementalBlast) and enemyCount < 4 and
          (not talent(4, 2) or (talent(4, 2) and -spell(SB.Stormelemental) < 120))
       then
        return cast(SB.ElementalBlast, target)
      end
