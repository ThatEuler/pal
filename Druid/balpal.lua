...
local function findHealer(name)
  ...
  if group_type ~= "solo" then
    ...
    for i = 1, (members - 1) do
      ...
      if
        (UnitGroupRolesAssigned(unit) == "HEALER") and not UnitCanAttack("player", unit) and not UnitIsDeadOrGhost(unit)
       then
        return unit
      end
    end
...
local function combat()
  ...
  --Starfall
  if
    modifier.lshift and talent(5, 1) and enemyCount >= aoeTarget and -spell(SB.Starfall) == 0 and castable(SB.Starfall) and
      power.astral.actual > 40
   then
  ...
  if modifier.lalt then
    ...
    if
      talent(3, 2) and player.buff(SB.Bearform).up and player.castable(SB.FrenziedRegeneration) and
        player.health.percent < 50
     then
  ...
  if
    GetShapeshiftForm() == 3 or player.buff(SB.Prowl).up or player.buff(SB.TigerDashBuff).up or player.buff(SB.Dash).up or
      not player.alive
   then
    return
  end
  ...
  if
    useTrinkets == "true" and GetItemCount(166793) == 1 and GetItemCooldown(166793) == 0 and
      ((player.buff(SB.CelestialAlignment).up or player.buff(SB.IncarnationBalance).up) or
        (-spell(SB.CelestialAlignment) > 30 or -spell(SB.IncarnationBalance) > 30))
   then
    macro("/use 13")
  end
  ...
  if
    autoPotion == "pot_b" and target.time_to_die > 10 and player.buff(burst).remains > 6 and GetItemCount(163222) >= 1 and
      GetItemCooldown(163222) == 0
   then
    macro("/use Battle Potion of Intellect")
    print("glug - battle potion of intellect - glug")
  end
  if
    autoPotion == "pot_c" and target.time_to_die > 10 and player.buff(burst).remains > 6 and GetItemCount(109218) >= 1 and
      GetItemCooldown(109218) == 0
   then
  ...
  if
    autoPotion == "pot_d" and target.time_to_die > 10 and player.buff(burst).remains > 6 and GetItemCount(152559) >= 1 and
      GetItemCooldown(152559) == 0
   then
  ...
  if player.buff(SB.FlaskofEndlessFathoms).down and not autoRune == "rune_a" then
    if
      autoRune == "rune_b" and
        (player.buff(SB.WhisperInsanityBuff).down or player.buff(SB.WhisperInsanityBuff).remains < 600) and
        GetItemCount(118922) == 1 and
        GetItemCooldown(118922) == 0
     then
  ...
  -- Interupts
  if
    toggle("interrupts", false) and target.interrupt(intpercent) and target.distance <= 45 and
      target.castable(SB.SolarBeam)
   then
  ...
  if
    autoDot and mouseover.castable(SB.Moonfire) and mouseover.alive and UnitAffectingCombat("mouseover") and
      mouseover.enemy and
      (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 6)
   then
  ...
  -- Moonkin Form
  if
    not modifier.lalt and UnitLevel("player") >= 20 and not player.spell(SB.MoonkinForm).lastcast and
      player.buff(SB.TigerDashBuff).down and
      GetShapeshiftForm() ~= 4
   then
  ...
  if toggle("opener", false) and y ~= 99 and arcanicPulsar == true and talent(5, 2) then
    ...
    if player.buff(SB.Starlord).count <= 2 and y == 1 then
      ...
      if
        target.castable(SB.Moonfire) and y == 1 and
          (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 6)
       then
        return cast(SB.Moonfire, "target")
      end
      if
        target.castable(SB.Sunfire) and y == 1 and
          (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 5)
       then
        return cast(SB.Sunfire, "target")
      end
      if
        talent(6, 3) and target.castable(SB.StellarFlare) and
          (not target.debuff(SB.StellarFlare).exists or target.debuff(SB.StellarFlare).remains < 7.2)
       then
        return cast(SB.StellarFlare, "target")
      end
      if
        target.castable(SB.LunarStrike) and player.buff(SB.LunarEmpowerment).count >= 1 and
          player.buff(SB.SolarEmpowerment).count == 0
       then
        return cast(SB.LunarStrike, "target")
      end
    ...
    if y == 2 and power.astral.actual < 40 then
      if
        target.castable(SB.Moonfire) and y == 2 and
          (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 6)
       then
        return cast(SB.Moonfire, "target")
      end
      if
        target.castable(SB.Sunfire) and y == 2 and
          (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 5)
       then
        return cast(SB.Sunfire, "target")
      end
      if
        talent(6, 3) and target.castable(SB.StellarFlare) and not lastcast(SB.StellarFlare) and
          (not target.debuff(SB.StellarFlare).exists or target.debuff(SB.StellarFlare).remains < 7.2)
       then
        return cast(SB.StellarFlare, "target")
      end
    ...
    if y == 2 and power.astral.actual < 80 then
      if
        target.castable(SB.Moonfire) and y == 3 and
          (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 6)
       then
        return cast(SB.Moonfire, "target")
      end
      if
        target.castable(SB.Sunfire) and y == 3 and
          (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 5)
       then
        return cast(SB.Sunfire, "target")
      end
  ...
  if toggle("opener", false) and not talent(5, 2) and y ~= 99 then
    ...
    if
      target.castable(SB.Sunfire) and
        (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 5)
     then
      return cast(SB.Sunfire, "target")
    end
    if
      target.castable(SB.Moonfire) and
        (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 6)
     then
      return cast(SB.Moonfire, "target")
    end
  ...
  if toggle("Heal", false) then
    if talent(3, 3) then
      -- Swiftmend
      if
        player.castable(SB.Swiftmend) and player.health.percent < 50 and
          (not player.buff(SB.MoonkinForm).exists or player.health.percent < 30)
       then
        return cast(SB.Swiftmend, player)
      end
      -- Rejuvenation
      if
        player.castable(SB.Rejuvenation) and player.health.percent < 75 and not player.buff(SB.MoonkinForm).exists and
          not (player.buff(SB.Rejuvenation).up or player.buff(SB.RejuvenationGermination).up)
       then
    ...
    if
      player.castable(SB.Regrowth) and
        ((player.health.percent < 48 and not player.buff(SB.Regrowth).up) or player.health.percent < 30)
     then
  ...
  if toggle("cooldowns", false) and target.time_to_die > 5 then
    if talent(5, 3) and power.astral.actual > 40 and -spell(SB.IncarnationBalance) == 0 then
      return cast(SB.IncarnationBalance)
    elseif
      player.castable(burst) and (not az_ls or player.buff(SB.LivelySpirit).up) and
        (player.buff(SB.Starlord).count >= 2 or not talent(5, 2))
     then
      return cast(burst)
    end

    if
      talent(7, 2) and -player.spell(SB.FuryofElune) == 0 and power.astral.actual <= 87 and
        (player.buff(burst).up or -spell(burst) > 30)
     then
  ...
  if
    talent(1, 3) and toggle("FON", false) and -spell(205636) == 0 and mouseover.alive and
      UnitAffectingCombat("mouseover") and
      mouseover.enemy and
      (player.buff(burst).remains > 10 or -spell(burst) > 30)
   then
    return cast(SB.ForceofNature, "ground")
  end
  ...
  if not modifier.shift and talent(5, 2) and target.castable(SB.Starsurge) and power.astral.actual >= 40 then
    if
      (player.buff(SB.Starlord).count < 3 or
        player.buff(SB.Starlord).remains >= 8 and player.buff(SB.ArcanicPulsar).count < 8) and
        enemyCount <= aoeTarget and
        (player.buff(SB.SolarEmpowerment).count + player.buff(SB.LunarEmpowerment).count) < 4 and
        player.buff(SB.SolarEmpowerment).count < 3 and
        player.buff(SB.LunarEmpowerment).count < 3 and
        (not az_ss or not player.buff(burst).up or not player.spell(SB.Starsurge).lastcast) or
        target.time_to_die <= (1.5 / ((UnitSpellHaste("player") / 100) + 1)) * power.astral.actual % 40 or
        power.astral.actual >= 87
     then
      return cast(SB.Starsurge, "target")
    end
  end
  ...
  if
    not modifier.shift and not talent(5, 2) and enemyCount < aoeTarget and target.castable(SB.Starsurge) and
      player.buff(SB.LunarEmpowerment).count <= 2 and
      player.buff(SB.SolarEmpowerment).count <= 2
   then
    return cast(SB.Starsurge, "target")
  end
  ...
  if
    target.castable(SB.Sunfire) and
      (not target.debuff(SB.SunfireDebuff).exists or target.debuff(SB.SunfireDebuff).remains < 3.6) and
      power.astral.actual < 87 and
      floor >= ceiling + 2 * enemyCount and
      (enemyCount > 1 or target.debuff(SB.Moonfire).up) and
      (not az_ss or not player.buff(burst).up or not player.spell(SB.Sunfire).lastcast)
   then
    return cast(SB.Sunfire, "target")
  end
  --moonfire
  ...
  if
    target.castable(SB.Moonfire) and
      (not target.debuff(SB.MoonfireDebuff).exists or target.debuff(SB.MoonfireDebuff).remains < 4.8) and
      target.time_to_die % (2 * UnitSpellHaste("player")) * enemyCount >= 6 and
      (not az_ss or not player.buff(burst).up or not player.spell(SB.Moonfire).lastcast)
   then
    return cast(SB.Moonfire, "target")
  end
  ...
  -- stellar
  if
    target.castable(SB.StellarFlare) and
      (not target.debuff(SB.StellarFlare).exists or target.debuff(SB.StellarFlare).remains < 7.2) and
      target.time_to_die % (2 * UnitSpellHaste("player")) >= 5 and
      (not az_ss or not player.buff(burst).up or not player.spell(SB.StellarFlare).lastcast)
   then
    return cast(SB.StellarFlare, "target")
  end
  ...
  if
    target.castable(SB.SolarWrath) and (player.buff(SB.StreakingStars).down or player.buff(SB.StreakingStars).count < 3) or
      player.buff(burst).down or
      not player.spell(SB.SolarWrath).lastcast
   then
    return cast(SB.SolarWrath, "target")
  end
...
return
  ...
  if GetShapeshiftForm() == 3 and player.moving then
    return
  elseif
    toggle("Forms", false) and not player.moving and UnitLevel("player") >= 20 and player.buff(SB.Prowl).down and
      player.buff(SB.MoonkinForm).down and
      player.buff(SB.TigerDashBuff).down and
...
local function interface()
  local settings = {
    ...
    template = {
      ...
      {
        type = "text",
        text = "Everything on the screen is LIVE.  As you make changes, they are being fed to the engine."
      },
      {type = "rule"},
      ...
      -- { key = 'input', type = 'input', text = 'TextBox', desc = 'Description of Textbox' },
      {
        ...
        type = "checkbox",
        text = "Healing Pot",
        desc = "Use Coastal Healing Potion if HS on CD/none"
      },
      {
        key = "intpercent",
        type = "spinner",
      ...
      {key = "autoRacial", type = "checkbox", text = "Racial", desc = "Use Racial on CD (Troll only)"},
      {
        ...
        type = "checkbox",
        text = "Arcanic Pulsar",
        desc = "this changes the rotation, do you have it?"
      },
