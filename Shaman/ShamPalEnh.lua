...
--    end
    ...
    -- Interrupts
        ...
        -- Wind Shear
        if toggle('interrupts', false) and castable(SB.WindShear, 'target') and -spell(SB.WindShear) == 0 and target.interrupt(intpercent, false) and target.distance < 30 then
...
if primalprimerbuild == true then
	...
	--Cast Crash Lightning if two or more targets are in range and the buff has less than 2 seconds remaining (or is inactive).
        if castable(SB.CrashLightning) and -spell(SB.CrashLightning) == 0 and enemyCount >= 2 
		and (not player.buff(SB.CrashLightning).up or player.buff(SB.CrashLightning).remains <= 2) then
...
if stormstrikebuild == true then
    ...
    --Cast Crash Lightning Icon Crash Lightning if 2 targets are in range and the buff is not present.
        if castable(SB.CrashLightning) and -spell(SB.CrashLightning) == 0 and enemyCount >= 2 and not player.buff(SB.CrashLightning).up then
            return cast(SB.CrashLightning)
        end
    ...
    --Cast Lightning Bolt if above 40 Maelstrom (50 with Fury of Air) with Overcharge talent.
        if castable(SB.LightningBolt) and -spell(SB.LightningBolt) == 0 and talent(4,3) and (player.power.maelstrom.actual > 40 or (player.power.maelstrom.actual > 40 and talent(6,2))) then
    ...
    --Cast Frostbrand with Hailstorm taken and the buff has less than 4.5 seconds remaining.
        if castable(SB.Frostbrand) and -spell(SB.Frostbrand) == 0 and player.buff(SB.Frostbrand).remains <= 4.5 and talent(4,2) then
            return cast(SB.Frostbrand)
        end

	--Cast Flametongue if the buff has less than 4.5 seconds remaining.
        if castable(SB.Flametongue) and -spell(SB.Flametongue) == 0 and player.buff(SB.Flametongue).remains <= 4.5 and not talent(4,1) then
            return cast(SB.Flametongue)
        end
	...
	--Cast Lava Lash if above 40 Maelstrom (50 with Fury of Air taken)
        if castable(SB.LavaLash) and -spell(SB.LavaLash) == 0 and (player.power.maelstrom.actual > 40 or (player.power.maelstrom.actual > 40 and talent(6,2))) then
            return cast(SB.LavaLash)
        end
