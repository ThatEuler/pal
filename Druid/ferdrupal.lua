...
--[[    if castable(SB.WarStomp, 'target') and -spell(SB.WarStomp) == 0 and target.interrupt(intpercent, false) then
  ...
  -- Apply Cat Form IC
    ...
    if UnitLevel("player") >= 110 then
        ...
        --Maintain Rip
        if castable(SB.Rip, 'target') and -spell(SB.Rip) == 0 and player.power.combopoints.actual >= 5 and target.debuff(SB.Rip).remains < 7 then
            return cast(SB.Rip)
        end
        ...
        if castable(SB.FerociousBite, 'target') and -spell(SB.FerociousBite) == 0 and player.power.combopoints.actual >= 5 and target.debuff(SB.Rip).remains < 10 then
    ...
    if UnitLevel("player") < 110 then
        ...
        if castable(SB.Rip, 'target') and -spell(SB.Rip) == 0 and player.power.combopoints.actual >= 5 and not -target.debuff(SB.Rip) then
            return cast(SB.Rip)
        end
        ...
        if UnitLevel("player") >= 90 and enemyCount >= 2 and castable(SB.BrutalSlash, 'target') and -spell(SB.BrutalSlash) == 0 then
            return cast(SB.BrutalSlash)
        end

        -- Thrash should be maintained on all targets when there are multiple enemies nearby.
        if UnitLevel("player") >= 12 and enemyCount >= 2 and castable(SB.Thrash, 'target') and -spell(SB.Thrash) == 0 and not -target.debuff(SB.Thrash) then
            return cast(SB.Thrash)
        end
        ...
        if UnitLevel("player") >= 32 and UnitLevel("player") < 90 and enemyCount >= 2 and castable(SB.Swipe, 'target') and -spell(SB.Swipe) == 0 then
