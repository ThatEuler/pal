...
if target.alive and target.enemy and player.alive and not player.channeling() then
    ...
    if castable(SB.DemonSpikes) and -spell(SB.DemonSpikes) == 0 and -player.health <= DSHealth and power.pain.deficit >= 30 then
    ...
    if talent (3,2) then
        ...
        if castable(SB.ImmolationAura, 'target') and -spell(SB.ImmolationAura) == 0 and target.distance < 8 and target.debuff(SB.FieryBrand).up then
          return cast(SB.ImmolationAura)
        end

        --actions.brand+=/fel_devastation,if=dot.fiery_brand.ticking
        if castable(SB.FelDevastation, 'target') and -spell(SB.FelDevastation) == 0 and target.distance < 8 and target.debuff(SB.FieryBrand).up and talent(6,3) then
    ...
    --actions.normal+=/spirit_bomb,if=soul_fragments>=4
        if castable(SB.SpiritBomb, 'target') and -spell(SB.SpiritBomb) == 0 and target.distance < 8 and buff(SB.SoulFragments).count >= SBSouls
                and power.pain.deficit <= 70 and talent(6,2) then
          return cast(SB.SpiritBomb, 'target')
        end

    --actions.normal+=/soul_cleave,if=!talent.spirit_bomb.enabled
        if castable(SB.SoulCleave, 'target') and -spell(SB.SoulCleave) == 0 and target.distance < 8 and buff(SB.SoulFragments).count >= SCSouls
                and power.pain.deficit <= 70 and not talent(6,2) then
          return cast(SB.SoulCleave, 'target')
        end

    --actions.normal+=/soul_cleave,if=talent.spirit_bomb.enabled&soul_fragments=0
        if castable(SB.SoulCleave, 'target') and -spell(SB.SoulCleave) == 0 and target.distance < 8 and buff(SB.SoulFragments).count == 0 and power.pain.deficit <= 70 then
          return cast(SB.SoulCleave, 'target')
        end

    --actions.normal+=/immolation_aura,if=pain<=90
        if castable(SB.ImmolationAura, 'target') and -spell(SB.ImmolationAura) == 0 and target.distance < 8 and power.pain.deficit >= 10 then
          return cast(SB.ImmolationAura)
        end

    --actions.normal+=/felblade,if=pain<=70
        if castable(SB.Felblade, 'target') and -spell(SB.Felblade) == 0 and target.distance < 8 and power.pain.deficit >= 30 and talent(3,3) then
          return cast(SB.Felblade)
        end

    --actions.normal+=/fracture,if=soul_fragments<=3
        if castable(SB.Fracture, 'target') and -spell(SB.Fracture) == 0 and target.distance < 8 and buff(SB.SoulFragments).count <= 3 and talent(4,3) then
...
local function interface()
    ...
    local settings = {
        ...
        template = {
            ...
            { key = 'FBHealth', type = 'spinner', text = 'Fiery Brand at Health %', default = '60', desc = 'cast Fiery Brand at', min = 0, max = 100, step = 1 },
            { key = 'MMHealth', type = 'spinner', text = 'Metamorphosis at Health %', default = '35', desc = 'cast Metamorphosis at', min = 0, max = 100, step = 1 },
            { key = 'DSHealth', type = 'spinner', text = 'Demon Spikes at Health %', default = '30', desc = 'cast Demon Spikes at', min = 0, max = 100, step = 1 },
            { key = 'SBHealth', type = 'spinner', text = 'Soul Barrier at Health %', default = '70', desc = 'cast Soul Barrier at', min = 0, max = 100, step = 1 },
            ...
            { key = 'SBSouls', type = 'spinner', text = 'Spirit Bomb at Souls#', default = '4', desc = 'cast Spirit Bomb at', min = 0, max = 5, step = 1 },
            { key = 'SCSouls', type = 'spinner', text = 'Soul Cleave at Souls#', default = '2', desc = 'cast Soul Cleave at', min = 0, max = 5, step = 1 },
        }
    }
