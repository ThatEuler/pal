...
local function combat()
    if player.alive then
        if player.buff(SB.TravelForm).exists then
            --or player.buff(SB.Refreshment).up or UnitChannelInfo("player") or player.buff(SB.Drink).up or player.buff(SB.ReplenishmentDebuff).up then
            return
        end
        ...
        if toggle('dispell', false) then
            ...
            if dispellable_unit and -spell(SB.NaturesCure) == 0 then
