...
local function combat()
  ...
  if
    toggle("interrupts", false) and target.interrupt() and target.distance < 8 and -spell(SB.Rebuke) > 0 and
  ...
  if
    target.enemy and target.distance < 8 and target.health.percent > 50 and GetItemCooldown(159627) == 0 and
  ...
  if
    lowest.castable(SB.BlessingofProtection) and lowest.health.percent <= 20 and lowest.debuff(SB.Forbearance).down and
      lowest ~= tank and
      lowest ~= player
   then
  ...
  if
    not talent(4, 3) and lowest.castable(SB.BlessingofSacrifice) and lowest.health.percent <= 40 and lowest ~= tank and
      lowest ~= player
   then
  ...
  if talent(1, 3) and castable(SB.ExecutionSentence, "target") then
    if
      enemyCount <= 2 and not talent(7, 2) or
        (enemyCount <= 2 and talent(7, 2) and player.buff(SB.Crusade).down and
          -spell(SB.Crusade) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2))
     then
  ...
  if castable(SB.DivineStorm) and -power.holypower >= 3 and enemyCount >= 2 then
    if talent(7, 1) and player.buff(SB.DivinePurpose).up then
      return cast(SB.DivineStorm)
    elseif
      talent(7, 2) and player.buff(SB.Crusade).down and
        -spell(SB.Crusade) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2)
     then
      return cast(SB.DivineStorm)
  ...
  if castable(SB.TemplarsVerdict, "target") then
    ...
    elseif
      talent(7, 2) and player.buff(SB.Crusade).down and
        -spell(SB.Crusade) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 3) and
        player.buff(SB.ExecutionSentence).down
     then
      return cast(SB.TemplarsVerdict, "target")
  ...
  local HoW = "foo"
  if
    not talent(2, 3) and target.health.percent >= 20 and
      (player.buff(SB.AvengingWrath).down or player.buff(SB.Crusade).down)
   then
    local HoW = "true"
  end
  ...
  elseif
    castable(SB.WakeofAshes) and -power.holypower == 1 and
      -spell(SB.BladeOfJustice) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 1)
   then
    return cast(SB.WakeofAshes)
  end
  ...
  elseif
    castable(SB.BladeOfJustice, "target") and -power.holypower == 3 and
      (-spell(SB.HammerofWrath) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2) or HoW == "true")
   then
    return cast(SB.BladeOfJustice, "target")
  end
  ...
  elseif
    castable(SB.Judgment, "target") and -power.holypower <= 4 and
      (-spell(SB.BladeOfJustice) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2) or HoW == "true")
   then
    return cast(SB.Judgment)
  end

  if
    target.enemy and -power.holypower <= 4 and target.castable(SB.HammerofWrath) and
      (target.health.percent <= 20 or (player.buff(SB.AvengingWrath).up or player.buff(SB.Crusade).up))
   then
  ...
  if talent(4, 2) and castable(SB.ConsecrationRet) then
    if
      -power.holypower <= 2 or
        (-power.holypower <= 3 and -spell(SB.BladeOfJustice) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2))
     then
      return cast(SB.ConsecrationRet)
    elseif
      -power.holypower == 4 and -spell(SB.BladeOfJustice) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2) and
        -spell(SB.Judgment) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2)
     then
      return cast(SB.ConsecrationRet)
    end
  end
  ...
  if castable(SB.CrusaderStrike, "target") then
    ...
    elseif
      -spell(SB.BladeOfJustice) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2) and
        -spell(SB.ConsecrationRet) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2) and
        -spell(SB.Judgment) > ((1.5 / ((UnitSpellHaste("player") / 100) + 1)) * 2)
     then
      return cast(SB.CrusaderStrike)
