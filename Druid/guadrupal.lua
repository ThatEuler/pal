...
local function combat()
  if target.alive and target.enemy and player.alive and not player.channeling() then
    ...
    -- Skull Bash
    if
      toggle("interrupts", false) and castable(SB.SkullBash, "target") and -spell(SB.SkullBash) == 0 and
    ...
    if
      toggle("interrupts", false) and castable(SB.MightyBash, "target") and -spell(SB.MightyBash) == 0 and
        -spell(SB.SkullBash) > 0 and
        target.interrupt(intpercent, false) and
    ...
    if
      toggle("interrupts", false) and castable(SB.IncapacitatingRoar, "target") and -spell(SB.IncapacitatingRoar) == 0 and
        -spell(SB.SkullBash) > 0 and
        -spell(SB.MightyBash) > 0 and
        target.interrupt(intpercent, false) and
        target.distance < 5
     then
    ...
    if player.buff(SB.IncarnationGuardianofUrsoc).up then
      ...
      if
        castable(SB.Maul) and -spell(SB.Maul) == 0 and power.rage.actual >= 90 and target.distance < 6 and
          enemyCount <= 3
       then
    ...
    if enemyCount == 1 or enemyCount == 2 then
      ...
      if
        castable(SB.Pulverize) and -spell(SB.Pulverize) == 0 and target.debuff(SB.Thrash).count == 3 and talent(7, 3) and
          target.distance < 6
       then
      ...
      if
        castable(SB.Moonfire) and -spell(SB.Moonfire) == 0 and player.buff(SB.GalacticGuardian).up and talent(5, 2) and
          target.distance < 40
       then
        return cast(SB.Moonfire, "target")
      end
    ...
    if enemyCount >= 3 then
      --Keep Moonfire ticking on your target
      if
        enemyCount == 3 and castable(SB.Moonfire) and -spell(SB.Moonfire) == 0 and target.debuff(SB.Moonfire).down and
          target.distance < 40
       then
        return cast(SB.Moonfire, "target")
      end
      ...
      --Use Moonfire with a Galactic Guardian proc, if talented
      if
        enemyCount == 3 and castable(SB.Moonfire) and -spell(SB.Moonfire) == 0 and player.buff(SB.GalacticGuardian).up and
          talent(5, 2) and
          target.distance < 40
       then
        return cast(SB.Moonfire, "target")
  ...
  local settings = {
    ...
    template = {
      ...
      {type = "text", text = "Defensive Settings"},
      {
        key = "BSHealth",
        type = "spinner",
        text = "Bark Skin at Health %",
        default = "30",
        desc = "cast Bark Skin at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "BFHealth",
        type = "spinner",
        text = "Bristling Fur at Health %",
        default = "30",
        desc = "cast Bristling Fur at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "RGHealth",
        type = "spinner",
        text = "Regrowth at Health %",
        default = "70",
        desc = "cast Regrowth at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "FRHealth",
        type = "spinner",
        text = "Frenzied Regen at Health %",
        default = "50",
        desc = "cast Frenzied Regen at",
        min = 0,
        max = 100,
        step = 1
      },
      {
        key = "SIHealth",
        type = "spinner",
        text = "Survival Instincts at Health %",
        default = "20",
        desc = "cast Survival Instincts at",
        min = 0,
        max = 100,
        step = 1
      }
