...
local function combat()
  ...
  and target.distance < 8 and gougeInt then
  ...
  if castable(SB.BladeFlurry) and -spell(SB.BladeFlurry) == 0 and enemyCount >= 2 and (toggle('ttdtoggle', true) or (target.time_to_die > 6 and toggle('ttdtoggle', false))) then
    return cast(SB.BladeFlurry, 'target')
  end

  --Cast Pistol Shot with Deadshot proc as quick as you can without overcapping
  if castable(SB.PistolShot) and -spell(SB.PistolShot) == 0 and player.buff(SB.Deadshot).up 
  and (player.power.combopoints.actual <= 4 or (player.power.combopoints.actual <= 3 and talent(1,1))) then
...
local function interface()
    ...
    local settings = {
        ...
        template = {
            ...
            { type = 'text', text = 'Blade Flurry has a Time-To-Die check to not waste it on low health mobs - this can be toggled on and off'},                       
            ...
            { key = 'rip', type = 'checkspin', text = 'Riposte', desc = 'Health % to cast at', default_check = true, default_spin = 60, min = 5, max = 100, step = 1 },
